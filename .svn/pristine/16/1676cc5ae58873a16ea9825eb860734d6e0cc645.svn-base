...
$(function(){
	$.ajax({
		...
		, success: function(obj){
			$(obj).each(function(i, item){
				...
				$('#song'+(i+1)+'songid').find('i').addClass("likeMusicPlus");   /* player에서 필요한 값 주기. */
				$('#song'+(i+1)+'songid').find('i').attr("songid",item.songid);
	...
	$.ajax({
		...
		, success: function(obj){
			$(obj).each(function(i, item){
				//likeMusicPlus songid
				$('#album'+(i+1)+'rank').html('<strong class="h4">Ranking # ' + item.ranking + '</h4>');
				$('#album'+(i+1)+'albumid').find('i').addClass("addALBList");   /* player에서 필요한 값 주기. */
				$('#album'+(i+1)+'albumid').find('i').attr("albumId",item.albumid);
...
/* i == 3 || i == 5 || i == 8 || i == 10 || i == 11 || i == 13 || i == 16 
|| i == 17 || i == 18 || i == 20 || i == 21 || i == 23 || i == 24 */
	/* $.each(["abc", "efg", "hij", "lmn", "opq"], function(i, value){
		if (i == 3-1) {
			$("#tile3title").text(value);			
		}
	}); */
	/* $('body').on('click', 'div[tile-id="2"] div.bottom.gd.bg-info.wrapper a', function(){
		$(this).text('asdasd');
		alert("tileid = 3");
	});
	$('body').on('click', 'div[tile-id="3"] a', function(){
		$("#tile3").text("asdasdasdasd");
	}); */
});
</script>
</head>
          ...
          <section class="scrollable w-f-md">
			...
			<c:if test="${not empty loginId }">
	          	<h4 class="font-thin m-l-md m-t">
		          	...
		          	<a href="/JukeBugs/"> <b>Rankings</b></a></h4>
		      </c:if>
		      <c:if test="${empty loginId }">
			      <h4 class="font-thin m-l-md m-t">
			          	<a href="/jb/"> <b>Rankings</b></a></h4>
		      </c:if>    	
           </div>
