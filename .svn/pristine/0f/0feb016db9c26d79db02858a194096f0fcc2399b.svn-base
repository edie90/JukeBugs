...
!function (a, b) {
	...
	jPlayerPlaylist = function (b, c, d) {
		var e = this;
		//this.getCurrentTime = a.event.jPlayer.status.currentTime,
		this.nowPlayended=0,
		//재생 시 기능 추가
		this.playFunc = function(getFunc){
			a(this.cssSelector.jPlayer).bind(a.jPlayer.event.play, getFunc);
		},
		//재생 종료 시 기능 추가
		this.ended = function (getFunc) {
			a(this.cssSelector.jPlayer).bind(a.jPlayer.event.ended, getFunc);
		},
		//셔플 선택 시 기능 추가
		this.shuffleOnFunc = function(getFunc){
			a(this.cssSelector.shuffle).click(getFunc);
		},
		//셔플 종료 시 기능 추가
		this.shuffleOffFunc = function(getFunc){
			a(this.cssSelector.shuffleOff).click(getFunc);
		},
		//현재 재생 번호 출력
		this.getPlayCount=0,
		this.current = 0,
		...
		a(this.cssSelector.jPlayer).bind(a.jPlayer.event.timeupdate, function () {
		}),
	...
	jPlayerPlaylist.prototype = {
			...
			_refresh : function (b) {
				...
				if (b && !a.isFunction(b))
					a(this.cssSelector.playlist + " ul").empty(), a.each(this.playlist, function (b) {
						a(c.cssSelector.playlist + " ul").append(c._createListItem(c.playlist[b], cnt))
						cnt+=1;
					}), this._updateControls();
				else {
					...
					a(this.cssSelector.playlist + " ul").slideUp(d, function () {
						...
						a.each(c.playlist, function (a) {
							d.append(c._createListItem(c.playlist[a], cnt))
							cnt+=1;
						}),
						c._updateControls(),
						a.isFunction(b) && b(),
			...
			_createListItem : function (b, listCount) {
				var c = this,
				d = "<li><div>";
				if (d += "<a href='javascript:;' id="+'"footerListRemove_'+listCount+'"'+" class='" + this.options.playlistOptions.removeItemClass + " '>&times;</a>", b.free) {
					var e = !0;
				...
				return d += "<a href='javascript:;' id="+'"footerListPlay_'+listCount+'"'+" class='" + this.options.playlistOptions.itemClass + "' tabindex='0' >" + b.title + (b.artist ? " <span class='jp-artist'>by " + b.artist + "</span>" : "") + "</a>",
				d += "</div></li>"
			},
			...
			add : function (b, c) {
				...
				c ? this.play(0) : 1 === this.original.length && this.select(0)
			},
			remove : function (c) {
				var d = this;
				//alert("inRemoveTest_02");
				...
				playTimeSet: function(n){
					a(this.cssSelector.jPlayer).jPlayer("play",n)
			},
				select : function (b) {
