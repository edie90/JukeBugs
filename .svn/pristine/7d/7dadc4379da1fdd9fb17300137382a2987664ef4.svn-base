...
<head>  
  ...
  <script type="text/javascript">
	  ...
	  function raidoGoSongName(content) {
		alert("bbb");
		alert(content);
				...
				<div class="row">
                  <div class="section_contin">
						...
						<div class="radio-change">
							<input type="radio" name="searchType" value="goArtistName" onclick="javascript:radioGoArtistName('${artistName}')">
								<a href="javascript:radioGoArtistName('${artistName}')">아티스트명에서</a>
							<input type="radio" name="searchType" value="goSongName" onclick="javascript:raidoGoSongName('${artistName}')">
								<a href="javascript:raidoGoSongName('${artistName}')">곡명에서</a>
							<input type="radio" name="searchType" value="goAlbumName" onclick="javascript:radioGoAlbumName('${artistName}')" checked="checked">
           ...
           <input type="hidden" id="currentPage" value="${pagenavi.currentPage}">
            <!-- 페이징 처리 -->
             <ul class="pagination pagination"> <!-- **************  -->
              ...
              <li><a href="goTotalAlbumNameSongList?currentPage=${pagenavi.currentPage-1}&content=${artistName}"><i class="fa fa-chevron-left"></i></a></li>
              
               <c:forEach var="counter" begin="${pagenavi.startPageGroup}" end="${pagenavi.endPageGroup}">
               	<c:choose>
             	  	 	<c:when test="${pagenavi.currentPage == counter}">
             			 	<li class="active"><a href="goTotalAlbumNameSongList?currentPage=${counter}&content=${artistName}">${counter}</a></li> <!-- **** -->
               	 		</c:when>
              		 	<c:otherwise>
							<li><a href="goTotalAlbumNameSongList?currentPage=${counter}&content=${artistName}">${counter}</a></li>                        
               	 		</c:otherwise>
               	</c:choose>
               	
               </c:forEach>
               
              <li><a href="goTotalAlbumNameSongList?currentPage=${pagenavi.currentPage+1}&content=${artistName}"><i class="fa fa-chevron-right"></i></a></li>
